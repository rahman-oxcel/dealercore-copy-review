...
const ids = $('section.tpl').map((i, e) => $(e).attr('id')).get();
...
const checks = {
  'template sections': $('section.tpl').length,
...
const problems = [];
if ($('section.tpl').length !== expected.length) problems.push('expected ' + expected.length + ' sections');
